...
local SPECIAL_GEOMETRY = {
    default = { width_pct = 75, height_pct = 75, y_pct = 5 },
    -- Per-workspace overrides (key = workspace name without "special:" prefix):
    phanpy = { width_pct = 40, height_pct = 80, y_pct = 5 },
}
...
local function get_special_config(ws_name)
    local d = SPECIAL_GEOMETRY.default
    local o = SPECIAL_GEOMETRY[ws_name] or {}
    return {
        width_pct  = o.width_pct  or d.width_pct,
        height_pct = o.height_pct or d.height_pct,
        y_pct      = o.y_pct      or d.y_pct,
    }
end

local function calc_special_pos(ws_name)
    local cfg = get_special_config(ws_name)
    local m   = get_monitor_info()
    local lw  = math.floor(m.width  / m.scale)
    local lh  = math.floor(m.height / m.scale)
    local w   = math.floor(lw * cfg.width_pct  / 100)
    local h   = math.floor(lh * cfg.height_pct / 100)
    local yo  = math.floor(lh * cfg.y_pct      / 100)
...
local function spawn_dropdown(term_cmd, cb)
    local pos    = calc_special_pos("scratchpad")
...
-- ============================================
-- SPECIAL WORKSPACE GEOMETRY
-- ============================================

hl.on("window.active", function(w)
    if not w.workspace or not w.workspace.special then return end
    local short_name = w.workspace.name:sub(9)  -- strip "special:" prefix
    local pos = calc_special_pos(short_name)
    local sel = "address:" .. w.address
    hl.dispatch(hl.dsp.window.float({ action = "enable", window = sel }))
    hl.dispatch(hl.dsp.window.resize({ x = pos.width, y = pos.height, relative = false, window = sel }))
    hl.dispatch(hl.dsp.window.move({ x = pos.x, y = pos.y, relative = false, window = sel }))
end)
    ...
    helpers.exec_async(
        ...
        function(_, _)
            pcall(function()
                ...
                helpers.delay(3, function()
                    hl.exec_cmd("/usr/lib/xdg-desktop-portal-gtk 2>/dev/null &")
                    hl.exec_cmd("/usr/libexec/xdg-desktop-portal-gtk 2>/dev/null &")
                end)
            end)
        end
    )
